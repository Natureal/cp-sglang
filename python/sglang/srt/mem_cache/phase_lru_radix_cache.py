...
class PhaseLRURadixCache(BasePrefixCache):
    ...
    def _evict_by_pred(self, num_tokens: int):
        ...
        while num_evicted < num_tokens and len(heap_by_pred):
            ...
            print(f"current ts = {self.current_ts}, len of key = {len(x.key)}, hash = {x.hash_value}, pred = {x.pred}, truth = {NRT[node.hash_value][0] if len(NRT[node.hash_value]) > 0 else 10000000}, features = {self.predictor.get_features(node.hash_value, self.current_ts)}")
