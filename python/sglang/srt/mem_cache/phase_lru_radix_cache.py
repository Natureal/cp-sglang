...
class PhaseLRURadixCache(BasePrefixCache):
    def __init__(
        ...
        self.evict_by_pred_count = 0
        self.evict_by_lru_count = 0
        ...
        self.lru_rank_sum = 0
        self.cached_item_ts = SortedList()
    ...
    def _insert_helper(self, node: TreeNode, key: List, value, finished_req):
        ...
        if len(key):
            ...
            print(f"insert_helper 3: {new_node.hash_value}")
    ...
    def _judge_evicted_in_phase(self, node: TreeNode):
        ...
        if node.hash_value in self.pred_evicted:
            error_flag = True
            rank = self.cached_item_ts.bisect_left(node.last_access_ts)
            ...
            self.lru_budget += 1
            print(f"current_ts = {self.current_ts}, len of self.U = {len(self.U)}, hash_value = {node.hash_value}, reset lru_budget = {self.lru_budget}, phase_err_param = {self.phase_err_param}")
        ...
        if error_flag == True and self.pred_evict_count > 0 and self.evict_by_pred_count > 0 and self.lru_evict_count > 0 and self.evict_by_lru_count > 0:
            pred_prob = self.pred_evict_count / self.evict_by_pred_count
            lru_prob = self.lru_evict_count / self.evict_by_lru_count
            print(f"pred error prob = {self.pred_evict_count} / {self.evict_by_pred_count} = {pred_prob}, lru error prob = {self.lru_evict_count} / {self.evict_by_lru_count} = {lru_prob}")
            k = (math.floor(pred_prob / lru_prob) - 1) * 3
            if lru_prob > pred_prob:
                k = -math.floor(lru_prob / pred_prob) * 3

            print(f"process k = {k}")
            self.lru_budget += k

        if self.pred_evict_count > 0 and self.lru_evict_count > 0:
            print(f"pred avg inv = {self.pred_rank_sum / self.pred_evict_count}, lru avg inv = {self.lru_rank_sum / self.lru_evict_count}")
    ...
    def _evict_by_lru(self, num_tokens: int, based_on_budget: bool):
        ...
        while num_evicted < num_tokens and len(leaves):
            ...
            if based_on_budget == True:
                ...
                self.evict_by_lru_count += 1
                self.lru_budget -= 1
                self.U.discard(x)
                
                self.cached_item_ts.discard(x.last_access_ts)
    ...
    def _evict_by_pred(self, num_tokens: int):
        ...
        for node in leaves:
            ...
            #evicted_in_phase_flag = 1 if node.hash_value in self.pred_evicted else 0
        ...
        while num_evicted < num_tokens and len(heap_by_pred):
            ...
            if len(x.parent.children) == 0 and x.parent != self.root_node and x.parent.lock_ref == 0:
                ...
                #evicted_in_phase_flag = 1 if x.parent.hash_value in self.pred_evicted else 0
    ...
    def _record_access(self, node: TreeNode, new_ts):
        ...
        if node.last_access_ts != 0:
            self.cached_item_ts.discard(node.last_access_ts)

        node.last_access_ts = new_ts
        self.cached_item_ts.add(new_ts)
