...
class LARURadixCache(BasePrefixCache):
    def __init__(
        self,
    ...
    def _insert_helper(self, node: TreeNode, key: List, value, finished_req):
        ...
        while len(key) > 0 and child_key in node.children.keys():
            ...
            if prefix_len < len(node.key):
                ...
                #print(f"insert_helper 1: {new_node.hash_value}, value: {new_node.match_tag}")
                ...
                if finished_req == True:
                    ...
                    node.match_tag -= 1
                    #print(f"insert_helper 2: {node.hash_value}, value: {node.match_tag}")
    ...
    def _generate_node_hash_value(self, node):
        if debug == False:
            ...
            node.hash_value = self._single_hash(node.prefix_key + [node.key[0]])
    ...
    def _judge_evicted_in_phase(self, node: TreeNode):
        ...
        if self.algo_type == "belady" or self.algo_type == "blindoracle":
            return

        if node.hash_value in self.pred_evicted:
            self.lru_budget += 1
            self.lru_candidate_ratio = self.lru_candidate_ratio / 2
            if debug == True:
                print(f"current_ts = {self.current_ts}, hash_value = {node.hash_value}, reset lru_budget = {self.lru_budget}, lru_candidate_ratio = {self.lru_candidate_ratio}")
    ...
    def set_algo_type(self, algo_type):
        if self.algo_type != algo_type:
            ...
            if algo_type == "lru":
                ...
                logger.info(f"Caching algorithm switches to laru")
    ...
    def set_label_type(self, label_type):
        if label_type == "binary":
            self.predictor.use_binary_label = True
    ...
    def _predict(self, nodes: List[TreeNode]):
        ...
        if len(node_to_pred) > 0:
            ...
            for i in range(len(node_to_pred)):
                node = node_to_pred[i]
                if self.predictor.use_binary_label == True:
                    node.pred = preds[i]
                else:
                    if preds[i] == 2**62:
    ...
    def _evict_by_pred(self, num_tokens: int):
        ...
        heap_by_lru = []
        for node in leaves:
            heapq.heappush(heap_by_lru, node)
        ...
        heap_by_pred = []
        candidate_num = max(int(len(leaves) * self.lru_candidate_ratio), 1)
        for i in range(candidate_num):
            node = heapq.heappop(heap_by_lru)
            heapq.heappush(heap_by_pred, (node.match_tag, -node.pred, node))

        num_evicted = 0
        while num_evicted < num_tokens:
            if len(heap_by_pred) < candidate_num:
                if len(heap_by_lru) == 0:
                    break
                node = heapq.heappop(heap_by_lru)
                heapq.heappush(heap_by_pred, (node.match_tag, -node.pred, node))
            ...
            if debug == True:
                print(f"eviction, current ts = {self.current_ts}, match_tag = {x.match_tag}, hash = {x.hash_value}, pred = {x.pred}, features = {self.predictor.get_features(x.hash_value, self.current_ts)}")
            ...
            if len(x.parent.children) == 0 and x.parent != self.root_node and x.parent.lock_ref == 0:
                ...
                heapq.heappush(heap_by_lru, x.parent)
...
def load_ks_qwen_data(tokenizer):
    ret = []
    if os.path.exists("wq_0627_qwen3_32b_6000.json"):
        with open('wq_0627_qwen3_32b_6000.json', 'r', encoding='utf-8') as f:
            conver_list = json.load(f)
            for conver in conver_list:
                for req in conver['conversations']:
                    ret.append(tokenizer.encode(req['content']))
                    #print(str(tokenizer.encode(req['content'])))
        print(f"total number of ks qwen reqs: {len(ret)}")
    else:
        print(f"file not found")
    return ret, 786000

def load_ks_deepseek_data(tokenizer):
    ret = []
    if os.path.exists("wq_0627_deepseek_r1_6000.json"):
        with open('wq_0627_deepseek_r1_6000.json', 'r', encoding='utf-8') as f:
            conver_list = json.load(f)
            for conver in conver_list:
                for req in conver['conversations']:
                    ret.append(tokenizer.encode(req['content'], truncation=True, max_length=131072))
                    #print(str(tokenizer.encode(req['content'])))
        print(f"total number of ks deepseek reqs: {len(ret)}")
    else:
        print(f"file not found")
    return ret, 5110000


def load_ks_qa_data(tokenizer):
    ret = []
    if os.path.exists("ks_qa.json"):
        with open('ks_qa.json', 'r', encoding='utf-8') as f:
            conver_list = json.load(f)
            for round in range(1, 1000):
                exist = False
                for conver in conver_list:
                    if len(conver['conversations']) >= round and conver['conversations'][round - 1]['from'] == "human":
                        exist = True
                        history = None
                        if round > 1:
                            for idx in range(round - 1):
                                if history is None:
                                    history = conver['conversations'][idx]['value']
                                else:
                                    history += conver['conversations'][idx]['value']

                        if history is None:
                            ret.append(tokenizer.encode(conver['conversations'][round - 1]['value']))
                        else:
                            ret.append(tokenizer.encode(history + conver['conversations'][round - 1]['value']))
                if exist == False:
                    break
        print(f"total number of ks qa reqs: {len(ret)}")
    else:
        print(f"file not found")
    return ret, 1112850
...
if __name__ == "__main__":
    ...
    tree = LARURadixCache(None, None, page_size=1, disable=False)
    #tree = GuardRadixCache(None, None, page_size=1, disable=False)

    importance_split = tree.predictor._model.model_.feature_importance(importance_type='split')
    importance_gain = tree.predictor._model.model_.feature_importance(importance_type='gain')
    feature_names = tree.predictor._model.model_.feature_name()

    # 打印每个特征及其重要性
    for idx in range(len(feature_names)):
        print(f"{feature_names[idx]}, split:{importance_split[idx]}, gain:{importance_gain[idx]}")

    #tree.set_algo_type("laru") #popu
    #tree.set_algo_type("lru")
    tree.set_algo_type("laru")
    #tree.set_algo_type("belady")
    #tree.set_algo_type("blindoracle")

    #tree.set_label_type("binary")

    json_str = '{"enable_online_training": "on", "training_interval": 10000, "training_window": 30000}'
    obj = json.loads(json_str, object_hook=lambda d: SimpleNamespace(**d))
    tree.set_online_training(obj)

    unique_keys = 0

    #sync_send_req_list, req_type_list = load_multiturn_data(tokenizer)
    #data_type = "multiturn"
    
    #sync_send_req_list, req_type_list = load_stress_test_data(tokenizer)
    #data_type = "stress"
    
    #sync_send_req_list, unique_keys = load_poisson_multiturn_data(tokenizer)
    #data_type = "poisson_multiturn"

    #sync_send_req_list, unique_keys = load_qwen_bailian_data()
    #data_type = "qwen_bailian"

    sync_send_req_list, unique_keys = load_ks_qa_data(tokenizer)
    data_type = "ks_qa"

    #sync_send_req_list, unique_keys = load_ks_qwen_data(tokenizer)
    #data_type = "ks_qwen"

    #sync_send_req_list, unique_keys = load_ks_deepseek_data(tokenizer)
    #data_type = "ks_deepseek"

    if tree.algo_type == "belady":
        current_ts = 0
        for req in sync_send_req_list:
            current_ts += 1
            ...
            prefix_hashes = tree._prefix_hash(req)
            for j in range(len(req)):
                if prefix_hashes[j] not in NRT:
                    NRT[prefix_hashes[j]] = deque()
                NRT[prefix_hashes[j]].append(current_ts)

    print(f"total distinct keys: {len(NRT)}")

    #hash = -4232634994979945749
    #print(f"debug: {NRT[hash]}")

    #total_size = unique_keys * 0.1
    #total_size = unique_keys * 0.05
    #total_size = unique_keys * 0.02
    total_size = unique_keys * 0.01
    #total_size = unique_keys * 0.005
    ...
    total_req_len = 0
    for idx in range(len(sync_send_req_list)):
        req = sync_send_req_list[idx]
    ...
    tree.pretty_print()
    print(f"online_training: {tree.predictor.enable_online_training}, delta_nums: {tree.predictor.delta_nums}, edc_nums: {tree.predictor.edc_nums}")
    print(f"total_size = {total_size}, req_count = {req_count}, total_req_len = {total_req_len}, algo = {tree.algo_type}, data type = {data_type}, label type = {tree.predictor.use_binary_label}")
