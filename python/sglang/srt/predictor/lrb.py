...
class LRBReuseDistancePredictor(ReuseDistancePredictor):
    def __init__(self, memory_window=1000000):
        ...
        this_ckpt_path = os.path.join(this_dir, f'delta{deltanums}_edc{edcnums}_label_{label_type}.txt')
    ...
    def _training_task(self):
        ...
        for address in self.feature_history:
            ...
            # the last one is a label
            if self. use_binary_label == True:
                self.tmp_features.append((*self.feature_history[address], self.current_ts - last_access_time, 1))
            else:
                self.tmp_features.append((*self.feature_history[address], self.current_ts - last_access_time, 2**62))
    ...
    def access(self, address, key_path_len, current_ts):
        ...
        if address not in self.access_time_dict:
            ...
            if self.use_binary_label == True:
                print(f"use binary label")
                self.features.append((*self.feature_history[address], current_ts - last_access_time, 0))
            else:
        ...
        self.feature_history[address] = [*[self.deltas[i][address] for i in range(self.delta_nums)], *[self.edcs[i][address] for i in range(self.edc_nums)], key_path_len]
    ...
    def predict(self, addresses, key_path_lens, current_ts):
        ...
        for k in range(len(addresses)):
            if addresses[k] not in self.access_time_dict:
                ...
                continue
            features.append((*[self.deltas[i][addresses[k]] for i in range(self.delta_nums)], *[self.edcs[i][addresses[k]] for i in range(self.edc_nums)], key_path_lens[k], current_ts - self.access_time_dict[addresses[k]][-1]))
